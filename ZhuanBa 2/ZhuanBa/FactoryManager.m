...
- (UIView *)createCellViewWithFrame:(CGRect)frame  imageName:(NSString *)imageName placeHolder:(NSString *)placeHoder imageTag:(NSInteger)imageTag textFiledTag:(NSInteger)textFiledTag cellHeight:(NSInteger)cellHeight target:(id)target isNeedImage:(BOOL)isNeedImage {
    ...
    if (isNeedImage) {
        contentImageView =  [[UIImageView alloc] initWithFrame:CGRectMake(10, 20, 25, 25)];
        contentImageView.image = image;
        contentImageView.tag = imageTag;
//        contentImageView.backgroundColor=[UIColor redColor];
        
        
        contentTextFiled = [[UITextField alloc] initWithFrame:CGRectMake(contentImageView.frame.origin.x + contentImageView.frame.size.width + 10, 0, kScreenWidth - 50 - contentImageView.frame.size.width-80, cellHeight)];
//        contentTextFiled.backgroundColor=[UIColor greenColor];
//        contentTextFiled.textColor=[UIColor blackColor];
        

        contentTextFiled.clearButtonMode = UITextFieldViewModeWhileEditing;
           contentTextFiled.returnKeyType = UIReturnKeyNext;
