...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        // 初始化 pagecontrol
        self.pageControl = [[UIPageControl alloc]initWithFrame:CGRectMake(width-230,height-30,100,30)];
...
- (void)setSlideImgArr:(NSMutableArray*)imgArr Interval:(NSTimeInterval)Interval
{
     ...
     */
    {
        ...
        [pageControl setCurrentPageIndicatorTintColor:ZS];
    ...
    {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[slideImagesArr objectAtIndex:i]]];
    ...
    // 取数组最后一张图片 放在第0页
    UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[slideImagesArr objectAtIndex:([slideImagesArr count]-1)]]];
    
    // 添加最后1页在首页 循环
    imageView.frame = CGRectMake(0, 0, width, height);
    [scrollView addSubview:imageView];
    // 取数组第一张图片 放在最后1页
    imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:[slideImagesArr objectAtIndex:0]]];
    // 添加第1页在最后 循环
